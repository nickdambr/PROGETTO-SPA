function eta_p = Propeller_Efficiency_lite(varargin)
...
eta_p =((integral(@(x)C_t_Func(x,J/pi,SigmaValue,Beta075Value),.1,1)).*J)./...
        (integral(@(x)C_p_Func(x,J/pi,SigmaValue,Beta075Value),.1,1));
        if eta_p<=0
            eta_p=0;
        end
